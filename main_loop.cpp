...
void dibujarLinea(float x0, float y0, float xf, float yf);
void calcLineaNeg(float x0, float y0, float xf, float yf);
void calcLineaPos(float x0, float y0, float xf, float yf);
...
void dibujarLinea(float x0, float y0, float xf, float yf) {
	...
	else if (m >= 0 && m <= 1) calcLineaPos(x0, y0, xf, yf);
	else if (m < 0 && m >= -1) calcLineaNeg(x0, y0, xf, yf);
...
void menu(void) {
	...
	while (opcion == 0) {
		...
		case 'l':
			opcion = 0;
			/*OBTENEMOS LOS VALORES DEL USUARIO.*/
			std::cout << "\n*NOTA: La coordenada de x0 debe ser positiva y menor a xf. ";
			std::cout << "\n\tIngresa x0: ";
			std::cin >> x0;
			std::cout << "\tIngresa y0: ";
			std::cin >> y0;
